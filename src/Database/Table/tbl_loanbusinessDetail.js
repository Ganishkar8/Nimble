...
const insertBusinessDetail = (
    ...
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                `INSERT OR REPLACE INTO ${tableName} (loanApplicationId,id,client_id,client_type,customerSubCatg,enterpriseShopName,udyamRegNum,dateofReg,dateofIncorp,dateofBusiness,year,month,industryType,industryLine,companyType,enterpriseType,businessLocation,noofEmployees,operatingdays,operatingtiming,bookKeepingStatus,homeBasedBusiness,custTransMode,timeByPromoter,npmRate,purchaseFrequency,typeofPurchase,salesFrequency,dmsId) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
                [loanApplicationId,
                    id,
                    client_id,
                    client_type,
