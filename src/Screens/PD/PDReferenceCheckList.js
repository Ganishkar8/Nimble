...
const PDReferenceCheckList = (props, { navigation }) => {
    ...
    const handleClick = (value, data) => {
        if (value === 'edit') {
            props.navigation.navigate('PDReferenceCheck', { referenceDetail: data })
        } else if (value === 'new') {
            props.navigation.navigate('PDReferenceCheck', { referenceDetail: 'new' })
    ...
    const deleteBankData = () => {
        ...
        apiInstancelocal(baseURL)
            .delete(`/api/v2/loan-demographics/BankDetail/${referenceID}`)
            .then(async response => {
                // Handle the response data
