...
const BusinessDocumentUpload = (props, { navigation }) => {
  ...
  const [businessImageUri, setBusinessImageUri] = useState(null);
  ...
  const [documentData, setDocumentData] = useState(
    global.LEADTRACKERDATA.applicantDocumentDetail,
  );
  ...
  const checkGeo = (
    ...
    const appDetails = {
      ...
      date: new Date(),
      g2: latitude + ',' + longitude,
      subcode: subCodeId,
    };

    apiInstance(baseURL)
      .post(`/api/v1/pd/PDBusinessVisit/checkGeo`, appDetails)
      .then(response => {
  ...
  const buttonNext = () => {
    ...
    documentList.map(item => {
      ...
      for (let i = 0; i < newDataArray.length; i++) {
        if (
          newDataArray[i].isDocumentMandatory &&
          !newDataArray[i].isImagePresent
        ) {
  ...
  const getAllDocuments = () => {
    ...
    apiInstance(baseURL)
      .post(`/api/v1/pd/PDBusinessVisit`, appDetails)
      .then(response => {
        // Handle the response data
        if (global.DEBUG_MODE)
          console.log('ResponseDataApi::' + JSON.stringify(response.data));
  ...
  const getDocuments = filteredDocument => {
    ...
    const documents = [
      {
        id: 1,
        genericName: 'Business Visit Image',
        ...
        parentId: 442,
        documentCategoryName: 'CAPTURE BUSINESS VISIT IMAGE',
        documentCategoryCode: 'CAPTURE BUSINESS VISIT IMAGE',
        workflowStageId: 504,
      },
      {
        id: 2,
    ...
    const organizedData = documents.reduce((acc, installment) => {
      ...
      if (existingCodeIndex !== -1) {
        ...
        if (filteredDocument.length > 0) {
          filteredData = filteredDocument[0].pdBusinessVisitChild.filter(
            item => item.documentType === installment.subCode,
          );
        }
        ...
        if (filteredData != undefined && filteredData != null) {
          if (filteredData.length > 0) {
            const extraJSON = {
              dmsID: filteredData[0].dmsId,
              isImagePresent: true,
              documentName: filteredData[0].documentName,
              imageId: filteredData[0].id,
            };
  ...
  const postDocuments = () => {
    ...
    const appDetails = {
      ...
      pdLevel: global.PDSTAGE,
      pageId: currentPageId,
      pdBusinessVisitChild: appDetailsFinal,
    };
    ...
    apiInstance(baseURL)
      .post(
        `/api/v1/pd/PDBusinessVisit/loan-application-number/${global.LOANAPPLICATIONNUM}/clientId/${global.CLIENTID}`,
        appDetails,
      )
      .then(response => {
        // Handle the response data ${item.clientId}
        if (global.DEBUG_MODE)
          console.log('PDDocumentUploadApi::' + JSON.stringify(response.data));
        setLoading(false);
  ...
  const onClickPreview = () => {
    props.navigation.navigate('PreviousDocumentPreview', {
      pageId: currentPageId,
      imageUri: businessImageUri,
      longitude: houseLongitude,
      latitude: houseLatitude,
      fromScreen: 'BusinessVisit',
    });
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <DeleteConfirmModel
        ...
        textContent={language[0][props.language].str_deletedesc}
        textClose={language[0][props.language].str_no}
        textDelete={language[0][props.language].str_yes}
