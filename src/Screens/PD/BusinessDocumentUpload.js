...
const BusinessDocumentUpload = (props, { navigation }) => {
    ...
    const getAllDocuments = () => {
        ...
        apiInstance(baseURL).post(`/api/v1/pd/PDBusinessVisit`, appDetails)
            .then((response) => {
                // Handle the response data
    ...
    const getDocuments = (filteredDocument) => {
        setLoading(false)
        const documents = [{ "id": 1, "genericName": "Business Visit Image", "genericType": "LOAN_DOCUMENT_TYPE", "subCode": "BUS_VIS_IMG", "isDocumentMandatory": true, "documentformatAccepted": [{ "isActive": true, "id": 185, "subCodeId": "PDF", "label": "PDF", "displayOrder": 0, "isDefault": 0 }, { "isActive": true, "id": 187, "subCodeId": "JPG/JPEG", "label": "JPG/JPEG", "displayOrder": 2, "isDefault": 2 }], "parentId": 442, "documentCategoryName": "CAPTURE BUSINESS VISIT IMAGE", "documentCategoryCode": "CAPTURE BUSINESS VISIT IMAGE", "workflowStageId": 504 },
        ...
        const organizedData = documents.reduce((acc, installment) => {
            ...
            if (existingCodeIndex !== -1) {
                ...
                if (filteredDocument.length > 0) {
                    filteredData = filteredDocument[0].pdBusinessVisitChild.filter(item => item.documentType === installment.subCode);
                }
    ...
    const postDocuments = () => {
        ...
        apiInstance(baseURL).post(`/api/v1/pd/PDBusinessVisit/loan-application-number/${global.LOANAPPLICATIONNUM}/clientId/${global.CLIENTID}`, appDetails)
            .then((response) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(BusinessDocumentUpload);
