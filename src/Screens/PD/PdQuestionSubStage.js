...
const PdQuestionSubStage = (props, { navigation }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      const filteredData =
        props.pdSubStage[0].personalDiscussionSubStageLogs.filter(
          data => data.subStageCode === global.PDSUBSTAGE,
        );

      const filteredModule =
        filteredData[0].personalDiscussionModuleLogs.filter(
  ...
  const navigateToQuestion = item => {
    ...
    } else if (item.subModuleCode == 'REFF_CHECK_CO_APPL') {
      if (global.PDSTAGE == 'PD_1') {
        pageId = 19;
      } else if (global.PDSTAGE == 'PD_2') {
        pageId = 56;
    ...
    } else if (item.subModuleCode == 'REFF_CHECK_GRNTR') {
      if (global.PDSTAGE == 'PD_1') {
        pageId = 31;
      } else if (global.PDSTAGE == 'PD_2') {
        pageId = 68;
