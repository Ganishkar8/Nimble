...
const HouseDocumentUpload = (props, { navigation }) => {
  ...
  const checkGeo = (
    ...
    apiInstance(baseURL)
      .post(`/api/v1/pd/PDHouseVisit/checkGeo`, appDetails)
      .then(response => {
    ...
    const documents = [
      {
        ...
        parentId: 442,
        documentCategoryName: 'CAPTURE HOUSE VISIT IMAGE',
        documentCategoryCode: 'CAPTURE HOUSE VISIT IMAGE',
        workflowStageId: 504,
      },
      {
        id: 2,
    ...
    const organizedData = documents.reduce((acc, installment) => {
      ...
      if (existingCodeIndex !== -1) {
        ...
        if (filteredDocument.length > 0) {
          filteredData = filteredDocument[0].pdHouseVisitChild.filter(
  ...
  const postDocuments = () => {
    ...
    apiInstance(baseURL)
      .post(
        `/api/v1/pd/PDHouseVisit/loan-application-number/${global.LOANAPPLICATIONNUM}/clientId/${global.CLIENTID}`,
        appDetails,
      )
      .then(response => {
